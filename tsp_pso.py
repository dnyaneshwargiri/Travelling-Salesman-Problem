		...
		for t in range(self.iterations):
			...
			for particle in self.particles:
				...
				if cost_current_solution < particle.getCostPBest():
					oldPbest=particle.getCostPBest()
					particle.setPBest(solution_particle)
					particle.setCostPBest(cost_current_solution)
					print('Iteration Number  :',t,'New PBest Cost  :',self.getGBest().getCostPBest())
...
if __name__ == "__main__":
	...
	pso = PSO(graph, iterations=100, size_population=15, beta=1, alfa=0.9)
	pso.run() # runs the PSO algorithm
	#pso.showsParticles() # shows the particles
	# shows the global best particle
	print('\n\nMinimum (Optimum) path to travel all cities is : %s | with a cost: %d\n' % (pso.getGBest().getPBest(), pso.getGBest().getCostPBest()))
